...
def execute_code(code, language, test_cases):
    ...
    try:
        ...
        results = {
            "test_results": [],  # This will store all test cases except last 10
            "remaining_passed": 0,  # For the last 10 test cases
            "remaining_failed": 0   # For the last 10 test cases
        }

        # Calculate the split point: all except last 10 test cases
        display_cases = test_cases[:-10]
        hidden_cases = test_cases[-10:]

        # Run displayed test cases (all except last 10)
        for test in display_cases:
            try:
                ...
                def normalize_output(output):
                    output = output.strip()
                    output = output.replace("Enter a string:", "").strip()
                    output = output.replace("Enter a string: ", "").strip()
                    output = output.replace("Enter text:", "").strip()
                    output = output.replace("Enter text: ", "").strip()
                    
                    # If the string is already wrapped in quotes, keep them
                    if not ((output.startswith('"') and output.endswith('"')) or \
                           (output.startswith("'") and output.endswith("'"))):
                        # If no quotes, wrap it in double quotes
                        output = f'"{output}"'
